				...
				{events.map((event) => {
					const getFormattedDate = (date: Date) => {
						try {
							...
							const formattedDate = dayjs.utc(date).tz(userTimeZone)
					...
					return (
						<Tr key={event._id.toString()}>
							<Td>{event.name}</Td>
							<Td>{getFormattedDate(event.startsOn)}</Td>
							<Td>{getFormattedDate(event.endsOn)}</Td>
							<Td>
								<Button onClick={() => router.push(`/console/bookings/${event._id}`)} colorScheme="blue" size="sm">
									View Bookings
								</Button>
							</Td>
						</Tr>
				...
				<Tr>
					<Td colSpan={7}>
						<Flex justify="center" align="center" mt={4}>
							<Button variant="link" onClick={handlePrev} disabled={pagination.page === 1} mr={2}>
								&lt;Prev
							</Button>
							<Text fontSize="sm" color="gray.600">
								Page {pagination.page} | showing {pagination.showing} of {pagination.total}
							</Text>
							<Button variant="link" onClick={handleNext} disabled={pagination.page === pagination.totalPages} ml={2}>
								Next&gt;
							</Button>
						</Flex>
					</Td>
