...
function GuestsList({ eventId }: { eventId: string }) {
	...
	return (
		<div className="max-w-4xl mx-auto bg-[#5656561e] border border-[#434343] rounded-2xl shadow-2xl overflow-hidden mt-8 py-3 px-6">
			...
			{isLoading && <p className="text-gray-400 text-sm">Loading guests...</p>}

			{!isLoading && guests?.data?.data?.length === 0 && <p className="text-gray-500 italic text-sm">No guests found for this event.</p>}
