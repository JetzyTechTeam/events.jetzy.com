  ...
  return (
    <>
      <div className="min-h-screen bg-gradient-to-br from-purple-900 to-indigo-900 py-8 px-4 sm:px-6 lg:px-7">
                ...
                {event.images.map((image, idx) => (
                  <div key={idx}>
                    <Image
                      src={image}
                      alt="Event Banner"
                      className="w-full md:h-[30rem] sm:h-52 object-cover object-top"
                    />
                  </div>
                ))}
              </Slider>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mb-6 space-y-4 sm:space-y-0">
              <div className="text-center sm:text-left">
                <p className="text-gray-600 text-sm sm:text-base mb-5">
                  ...
                  {new Date(event.startsOn.toString()).toLocaleTimeString()}
                </p>
                <p className="text-gray-600 text-sm sm:text-base mb-5">
                  {event?.timezone || ""}
                </p>
              ...
              <div className="flex flex-col items-center sm:items-end">
                <a
                  ...
                  className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-6 py-3  whitespace-nowrap rounded-full hover:from-purple-700 hover:to-indigo-700 transition-all transform hover:scale-105 shadow-lg"
                >
                  <TicketIcon className="w-6 h-6 inline-block mr-2" /> Get
                  Tickets
                </a>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                <h3 className="font-semibold text-lg text-gray-800 mb-4">
                  <CalendarDateRangeIcon className="w-6 h-6 inline-block mr-2" />{" "}
                  Schedule Date and Time
                </h3>
                <p className="text-gray-600 my-3">
                  <span className="text-gray-500 font-bold">From:</span>{" "}
                  {new Date(event.startsOn).toDateString()}{" "}
                  {new Date(event.startsOn).toLocaleTimeString()}
                </p>

                <p className="text-gray-600 mt-1">
                  <span className="text-gray-500 font-bold">To:</span>{" "}
                  {new Date(event.endsOn).toDateString()}{" "}
                  {new Date(event.endsOn).toLocaleTimeString()}
                </p>
              </div>
        ...
        {/* Participants List */}
     {event.showParticipants &&  <div className="max-w-4xl mx-auto mt-8 bg-white/90 backdrop-blur-lg rounded-2xl shadow-2xl overflow-hidden transform transition-all p-6">
          <h3 className="font-semibold text-lg text-gray-800 mb-4">
            Participants
          </h3>
          {loading ? (
