		...
		<ConsoleLayout>
			<div className="max-w-5xl mx-auto flex gap-x-5">
				<div className="w-[360px]">
					<Image src={data.images[0]} alt={data.name} width={360} height={180} className="w-[360px] h-[180px] rounded-xl object-cover" />
				...
				<div className="w-[760px]">
					<Heading className="text-[38px] font-semibold text-[#FFFFFFC2]">{data.name}</Heading>

					<Flex justifyContent="space-between" alignItems="center" className="relative bg-[#FFFFFF0A] rounded-xl mt-5 p-3">
						<Box>
						...
						<Box className="space-y-2">
							<Flex gap={5} alignItems="center">
								<Box bg="#1D1F23" p="2" rounded="lg" w="80px" textAlign="center">
									<Text>{new Date(data.startsOn).toTimeString().slice(0, 5)}</Text>
								</Box>
								<Box bg="#1D1F23" p="2" rounded="lg" w="175px">
									<Text>{new Date(data.startsOn).toISOString().slice(0, 10)}</Text>
								</Box>
							</Flex>
							<Flex gap={5}>
								<Box bg="#1D1F23" p="2" rounded="lg" w="80px" textAlign="center">
									<Text>{new Date(data.endsOn).toTimeString().slice(0, 5)}</Text>
								</Box>
								<Box bg="#1D1F23" p="2" rounded="lg" w="175px">
									<Text>{new Date(data.endsOn).toISOString().slice(0, 10)}</Text>
								</Box>
							</Flex>
						</Box>
					</Flex>
					...
					<Flex alignItems="center" gap="2" bg="#FFFFFF0A" rounded="lg" p="3" mt="5">
						<LocationSVG />
						<Text className="font-medium text-[#FFFFFFA3]">{data.location}</Text>
					</Flex>
					...
					<Flex gap="2" bg="#FFFFFF0A" rounded="lg" p="3" mt="5">
						...
						<Text className="font-medium text-[#FFFFFFA3]">{data.desc.slice(0, 200)}...</Text>
					</Flex>
				...
				<ModalContent bg="#1E1E1E">
					...
					<ModalBody pb={6}>
						<FormControl>
							...
							<Input required bg="#090C10" border="1px solid #444444" placeholder="Full Name" value={name} onChange={(e) => setName(e.target.value)} />
						</FormControl>
