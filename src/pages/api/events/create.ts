...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
	...
	try {
		...
		let { startDate, startTime, endDate, endTime, name, location, venueName, longitude, latitude, placeId, capacity, requireApproval, images, tickets, isPaid, desc, privacy, timezone, showParticipants, interestCategory, interestSubCategory, host, invitedGuests } = params
		...
		try {
			...
			console.error("[events/create] Timezone conversion error:", timezoneError)
		...
		const newEvent = await Events.create({
			...
			tickets: tickets.map((ticket, index) => {
				// Auto-disable logic: Check if tickets should be disabled based on due date
				const now = new Date()
