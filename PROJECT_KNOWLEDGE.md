...
- Open with the backend: `source` is logged, not stored (asked for `signupSource`, set on create only); existing album stub accounts are not backfilled (not needed).
