...
- **Promoted events on the album page:** [PromotedEvents.tsx](src/components/events/album/PromotedEvents.tsx) — `usePromotedEvents(excludeEventId)` (one `useQuery` on `/api/events?page=1`, filtered to **live + future** via `getEventStatus`, this album's own event dropped), `PromotedEventCard`, `PromotedEventsRail`. Desktop: up to 4 in the sticky left column (which gained `maxH`/`overflowY` so the rail can't clip). Mobile: one card interleaved **at row boundaries** whenever the running tile count crosses a multiple of 2 — never inside a row, because a 2-up row is a flex `row` from `sm` up. Each side is hidden at the other's breakpoint so the same events never show twice. `/api/events` has no `limit` param (LIMIT 20, hardcoded) so the slice is client-side, and `search` is never passed — that path makes an outbound call to the Jetzy interests API.
