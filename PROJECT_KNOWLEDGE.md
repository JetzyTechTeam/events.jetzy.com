...
- **Ended events lead with the photos:** once `getEventStatus(event) === "past"`, the event page collapses **Description** and (for host/admin, the only ones who still see it) **Tickets** behind chevron toggles, closed by default, and the album container widens from `max-w-4xl` to `max-w-6xl`. A live event renders exactly as before — no toggles, no width change. Guests never saw tickets on an ended event; that rule is unchanged.
