...
- **Host emails (2026-09-21).** The owner is emailed when the event ENTERS the review queue (`sendEventSubmittedForReview`) and when an admin approves it (`sendEventApprovedByAdmin`). Always through **[src/lib/event-approval-notify.ts](src/lib/event-approval-notify.ts)** (server only, best-effort). "In the queue" = `isAwaitingAdminReview` (pending **and not a draft** — the create page autosaves a pending draft nobody has submitted). Sent on the **transition** only: `create.ts` (published straight away), `update.ts` + `details.ts` (`!before && after` — draft published, or private→public). `approve.ts` flips with a conditional `findOneAndUpdate({adminApprovalStatus:"pending"})` so only the request that actually flipped it emails. Entering review ALSO emails `ADMIN_SUPPORT_EMAIL` (`sendEventReviewAdminNotice`, `replyTo` = host, link to the manage page's Approve) — same trigger, same non-admin-owner rule. `clone.ts` now sets `adminApprovalStatus` like `create.ts` (public → pending); it used to take the schema default `approved`, so a cloned public event went live unreviewed. **Non-admin owners only**, judged by the OWNER's account role, not the session — an admin approving a host's event still reaches the host.
...
- **Moving existing members to the new price: `scripts/reprice-premium-subscriptions.ts`** (2026-09-21). A price change never moves an existing subscription. The script swaps each `active`/`trialing` Premium subscription's item onto the current price for its interval with `proration_behavior: "none"` — nothing is invoiced or credited now; the period already paid runs out at the old rate and the next payment is at the new one (CEO decision). Skips cancelling-at-period-end, `past_due`, `unpaid`, multi-item. Dry-run by default; `--apply`, `--subscription sub_x`, `--limit N`. It stamps `metadata.repricedFrom`, which makes the webhook (a) send **no** plan-change email for the swap and (b) add a "we've lowered the price — you paid $10 instead of $20" line to the **first** renewal email that actually charges the new rate, then set `metadata.repriceAnnounced`. The member hears about it when money moves, never before — so a cancellation or failed card in between means they are told nothing about a price they never paid.
...
- **Leaving Manage Event with an unpublished draft now interrupts (2026-09-26).** The shadow draft is invisible to guests and the only notice was the orange banner on the host's NEXT visit, so hosts edited, left, checked the event page and reported their changes "weren't saved". `src/hooks/useUnsavedDraftGuard.ts` blocks the departure: in-app navigation is aborted the documented pages-router way (`routeChangeError` + throw) and the page renders its own dialog — **Update Event** / **Leave as draft** / **Keep editing**; tab close and reload get the browser's own prompt via `beforeunload`, whose wording is not ours to set. **Published events only** — a draft event autosaves in place and was never live.
  - The guard reads the **autosave pill's status** (`unsaved`/`saving`) plus a `hasPendingDraft` flag, **never Formik's `dirty`**: the mount effect seeds tickets and media with `setFieldValue`, so `dirty` can be true with no edit behind it.
  - **Aborting a popstate navigation leaves the address bar on the page you refused to go to**, so the hook re-stamps `router.asPath` with `history.pushState` inside the abort.
  - `bypass()` suppresses one departure and is cleared on `routeChangeComplete`/`routeChangeError` — Discard draft does `router.replace(router.asPath)`, which reloads props without remounting, and a bypass that stuck would kill the guard for the rest of the page's life. Called by Discard draft, Clone, Delete and the successful save.
  - **The form was DIRTY the moment any event with tickets loaded (fixed 2026-09-26).** `initialValues` seeded tickets with ten keys and the mount effect then overwrote them with five (`id/title/price/description/requireApproval`); react-fast-compare counts own keys, so Formik reported `dirty`, the pill read "Unsaved changes" on a page nobody had touched, and an autosave fired ~2s later — **creating a shadow draft on an untouched event**, which is the stale-draft trap of 2026-09-01 all over again. There is now ONE mapper per source (`mapEventTicket` / `mapDraftTicket`) and the mount effect no longer touches tickets at all: `initialValues` + `enableReinitialize` already seed them, and writing them again was the whole bug. The effect keeps only the media state, which lives outside Formik.
  - **`draftRevision.payload.intendedStatus` carries the host's Status choice.** `payload.status` is always `"draft"` (autosave forces it so a shadow-draft write can never publish), so it could not carry what the host picked: set Status to Draft, leave, come back, and the dropdown read Published again — the one field a draft silently forgot. Absent (drafts written before this) still means published. `update.ts` destructures named keys, so the extra field is inert on a real save.
